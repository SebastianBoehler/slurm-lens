...
export function timeline(frame, compact=false) {
  const allocated = frame.jobs.filter(j=>j.node && j.started_at);
  ...
  const start = Math.floor(earliest/3600000)*3600000;
  const hours = (end-start)/3600000;
  const height = compact ? 300 : Math.max(380,Math.min(740,hours*68));
  const top = date => Math.max(0,Math.min(height,(new Date(date).getTime()-start)/(end-start)*height));
  if (!nodes.length) return '<div class="empty">No node allocations are recorded in this snapshot.</div>';
  const ticks=[];
  for(let t=start;t<end;t+=3600000) ticks.push(`<div class="time-tick" style="top:${top(t)}px"><span>${time(t)}</span></div>`);
